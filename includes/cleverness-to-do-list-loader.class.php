...
class CTDL_Loader {
	...
	public static function create_taxonomies() {
		...
		register_taxonomy(
			'todocategories', array( 'todo' ), array(
				'hierarchical' => true,
				'labels'       => $labels,
				'show_ui'      => false,
				'query_var'    => false,
	...
	private static function include_files() {
		...
		if ( self::$settings['categories'] == 1 ) {
			include_once CTDL_PLUGIN_DIR . 'includes/cleverness-to-do-list-categories.class.php';
		}
		if ( is_admin() ) {
	...
	private static function call_wp_hooks() {
		if ( self::$settings['admin_bar'] == 1 ) {
			add_action( 'admin_bar_menu', array( 'CTDL_Lib', 'add_to_toolbar' ), 999 );
		}
		if ( is_admin() ) {
			...
			if ( self::$settings['categories'] == 1 ) {
				add_action( 'admin_init', array( 'CTDL_Categories', 'initialize_categories' ) );
			}
		}
	...
	public static function create_admin_menu() {
		...
		$cleverness_todo_page = add_menu_page(
			apply_filters( 'ctdl_todo_list', __( 'To-Do List', 'cleverness-to-do-list' ) ), apply_filters( 'ctdl_todo_list', __( 'To-Do List', 'cleverness-to-do-list' ) ),
			self::$settings['view_capability'], 'cleverness-to-do-list', array( __CLASS__, 'plugin_page' ), apply_filters( 'ctdl_icon', 'dashicons-yes' )
		);
		if ( self::$settings['categories'] == 1 ) {
			$cleverness_todo_cat_page = add_submenu_page(
				'cleverness-to-do-list', apply_filters( 'ctdl_categories_title', __( 'To-Do List Categories', 'cleverness-to-do-list' ) ),
	...
	public static function add_admin_js() {
		...
		wp_localize_script( 'cleverness_todo_js', 'ctdl', self::get_js_vars() );
	}
	...
	public static function frontend_admin_enqueue_scripts() {
		...
		wp_localize_script( 'cleverness_todo_frontend_admin_js', 'ctdl', self::get_js_vars() );
	}
	...
	public static function frontend_checklist_enqueue_scripts() {
		...
		wp_localize_script( 'cleverness_todo_checklist_complete_js', 'ctdl', self::get_js_vars() );
	}
